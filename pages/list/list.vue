<template>
  <view class="uni-tab-bar">
    ...
    >
      <swiper-item
        v-for="(tabItem, tabIndex) in newsList"
        :key="tabIndex"
      >
        <scroll-view
          class="list"
        ...
        >
          <block
            v-for="(newsItem, newsIndex) in tabItem.data"
            :key="newsIndex"
          >
            <list-block
              :newsItem="newsItem"
              @goDetail="goDetail(newsItem)"
            ></list-block>
          </block>
        </scroll-view>
      </swiper-item>
...
<script>
import listBlock from "@/pages/list/list-block";
    ...
    this.tabBars.forEach(tabBar => {
      this.newsList.push({
        ...
        requestParams: {
          ...
          pageSize: 10,
          column:
            "id,post_id,title,author_name,cover,published_at,comments_count"
        },
        loadingText: "加载中..."
      });
    });
